...
const supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { telemetry: false });
...
function goBackFromAllianceTeamPage(){
    document.getElementById("alliancesutomquestionslist").innerHTML = "";
...
function goBackFromTeamPagePreScout(){
    unlockAndClearPrescoutInputs();
    clearCurrentPath();
    document.getElementById("customquestiondiv").innerHTML = "";
    document.getElementById("prescoutteamslist").style.display = "flex";
...
async function submitNewQuestion(){
    ...
    if(abilityCount < 11){
        ...
        statusPopUp("Successfully added new question!");
        closeNewQuestionPopup();
    }else{
        console.error('Too many custom questions, maximum of 10');
        statusPopUp("Too many custom questions. max: 10");
        return;
    }
}
...
async function getMatchList() {
    try {
        ...
        if (!emptyError && emptyData === true) {
            ...
            let superTable = {};
            matches.forEach(m => {
                superTable[m.matchKey] = JSON.parse(JSON.stringify(scoreTableTemplate));
            });
